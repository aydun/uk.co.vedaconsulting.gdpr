...
<script type="text/javascript">
  CRM.$(function($){
    ...
    groupChk.each(function() {
      checkGroupChannels(this, false)
    });
    groupChk.on('change', function() {
      checkGroupChannels(this, true);
    });

    function checkGroupChannels(controller, displayMsg) {
        ...
        if (mismatchedChannels.length !== 0) {
         ...
         if (displayMsg) {
          CRM.confirm({
            ...
            message: ts('%1 has to be enabled for this newsletter',  {1: '<em>' + mismatchedChannelTxt + '</em>'})
          })
